 ...
 */
@Singleton
public class JsonToOwner implements Function<RawOwner, Owner> {
    @Override
    public Owner apply(@Nullable RawOwner input) {
        if(input == null){
            return null;
        }
        ...
        if(input.email != null){
            ownerBuilder.email(input.email);
        }

        if(input.uuid != null){
            ownerBuilder.uuid(input.uuid);
        }
