...
public class CloudSigmaComputeServiceAdapter implements
   ...
   private static final Predicate<DriveInfo> PREINSTALLED_DISK = Predicates.and(Predicates.notNull(),
         new Predicate<DriveInfo>() {
            ...
            public boolean apply(DriveInfo drive) {
               return drive.getMedia().equals(MediaType.CDROM);
            }
   ...
   public Iterable<DriveInfo> listImages() {
      return FluentIterable.from(transformParallel(client.listDrivesInfo(),
            new Function<DriveInfo, ListenableFuture<? extends DriveInfo>>() {

               @Override
               public ListenableFuture<DriveInfo> apply(DriveInfo input) {
                  try {
   ...
   public void destroyNode(String id) {
      ...
      if (server != null) {
         ...
         for (ServerDrive dev : server.getDrives())
            client.deleteDrive(dev.getDrive().getUuid());
      }
   }
