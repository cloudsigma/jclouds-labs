 ...
 */
@Test(groups = "unit")
public class ParseIPInfoTest extends CloudSigma2ParserTest{

    private static final ParseIPInfo PARSE_IP_INFO = Guice.createInjector(new GsonModule()).getInstance(ParseIPInfo.class);
    ...
    public void test(){
        Assert.assertEquals(PARSE_IP_INFO.apply(HttpResponse.builder()
                                                    .message("OK")
                                                    .statusCode(200)
                                                    .payload(getJsonResource("/ips-single.json"))
                                                    .build())
        , expected);
    }
}
